...
#define SWUART_BAUD_RATE          9600         //note: 跑 HSE 120M Hz , TX 成功跑 115200 bps
...
static void swuart_init(void)
{
    ...
    // 計數滿半個 bit 時間就中斷
    //** note: STM32f205 timer2 受到 APB1 推動，每一個 H-L clock 推動 2 次 (count).
...
static void swuart_send_byte(uint8_t b)
{
    ...
    uint32_t bitcnt = (4*clock_setup.apb1_frequency / SWUART_BAUD_RATE);
...
static void delay_ms(uint32_t ms)
{
    for(; ms > 0; ms--)
    {
        //for stm32f205@120 MHz, offset 0，ms = 10，實測 9.78 ms
        DWT_CYCCNT = 0;
        for(; DWT_CYCCNT < (4*clock_setup.apb1_frequency/1000);)
        { } //1 ms
    }
}
